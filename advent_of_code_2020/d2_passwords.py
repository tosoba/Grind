...
class PasswordWithPolicy:
    ...
    def __init__(self, password_with_policy_str: str) -> None:
        policy_range_str, policy_char_str, password = password_with_policy_str.strip().split(' ')
        policy_min, policy_max = policy_range_str.split('-')
        self.__policy_max = int(policy_max)
        self.__policy_min = int(policy_min)
        self.__policy_char = policy_char_str[0]
        self.__password = password

    def is_valid_1(self) -> bool:
        return self.__policy_min <= self.__password.count(self.__policy_char) <= self.__policy_max

    def is_valid_2(self) -> bool:
        return (self.__password[self.__policy_min - 1] == self.__policy_char) ^ (
                self.__password[self.__policy_max - 1] == self.__policy_char)


password_with_policy_str_input = ['14-15 v: vdvvvvvsvvvvvfpv',
...
def count_valid() -> int:
    valid_count = 0
    for str_input in password_with_policy_str_input:
        if PasswordWithPolicy(str_input).is_valid_2():
            valid_count += 1
    return valid_count
