...
class Codec {
    private fun nodesAtDepth(depth: Int): Int = 2.0.pow(depth.toDouble()).toInt()

    private var currentMaxDepth = 0
    private fun maxDepth(root: TreeNode?, currentDepth: Int = 0) {
        if (root == null) return
        currentMaxDepth = max(currentDepth, currentMaxDepth)
    ...
    fun serialize(root: TreeNode?): String {
        ...
        var currentNodeAtDepth = 0
        while (depth <= currentMaxDepth) {
            val node = queue.poll()
            if (node == null) result.append("n") else result.append(node.`val`)
            result.append(" ")
            queue.addLast(node?.left)
            queue.addLast(node?.right)
            if (++currentNodeAtDepth == nodesAtDepth(depth)) {
                ++depth
                currentNodeAtDepth = 0
            }
        }
    ...
    fun deserialize(data: String): TreeNode? {
        val spl = data.split(" ")
        if (spl.isEmpty() || (spl.size == 1 && spl.first() == "n")) return null

        val vals = spl.map { if (it == "n") null else it.toIntOrNull() }
